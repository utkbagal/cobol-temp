...
@router.post("/claim_ingest")
async def ingest(
    file: UploadFile = File(...),
    payload: str = Form(...) # Receive payload as a string
):
    filename = file.filename
    payload_data = json.loads(payload)
    cid = payload_data.get('cid')
    content = await file.read()
    track_event("ingestion_started",None,{"filename":filename})

    cid = await ingest_document(filename,content,'claim',cid)

    track_event("ingestion_completed",None,{"filename":filename})
...
@router.post("/pol_ingest")
async def ingest(file: UploadFile = File(...)):
    filename = file.filename
    content = await file.read()
    track_event("ingestion_started",None,{"filename":filename})

    cid = await ingest_document(filename,content,'policy')
...
#@router.get("/retr_docs")
#async def retrieve(cid: str = Body(...)):
#    cid = cid.get('cid')
...
async def process_claim(payload: dict = Body(...)):
    ...
    if detected_intent not in ("qna","unknown","view_policy","claim_status"):
